...
Successfully admitted images returned by a Tool remain visible as transcript output when Compact mode collapses that Tool receipt. Expanding the receipt restores its native inline gallery without duplication. This projection follows the durable image content shape rather than a Tool name, so browser captures and future image providers share the contract.
