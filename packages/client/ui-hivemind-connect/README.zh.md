...
此包还提供键为 `hivemind_connected_task` 的工具视图。其持久化结果会先投影每个有界的 Composio 操作，包括搜索和连接管理，再显示连接、审批、完成或失败卡片。Host 等待连接授权时，同一工具视图会在进度行后以内联方式渲染待处理交互；常驻输入框继续挂载在对话记录下方。重放从已记录的工具结果生成已结束的进度行，实时授权控件则由 Session 待处理交互服务持有。
