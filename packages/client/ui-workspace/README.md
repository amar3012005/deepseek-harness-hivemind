...
- **HIVE recent-session actions** — persisted HIVE rows reuse the native menu for Rename, Fork, Share, and Archive. Share uses the stable session URL and falls back to copying it when Web Share is unavailable. Archive preserves the append-only log.
