...
async function bench() {
  ...
  ctx.slots.register({
    ...
    children: {
      ...
      'tool.call.inline-images': { kind: 'single', scope: 'session' },
    },
  } as never, () => null)
...
describe('attachment plugin', () => {
  ...
  it('registers all entries and removes them with the plugin fiber', async () => {
    ...
    expect(ctx.slots.entries('tool.call.inline-images')).toMatchObject([{
      locale: 'conversation',
      component: MessageImages,
    }])
    ...
    expect(ctx.slots.entries('tool.call.inline-images')).toHaveLength(0)
  })
})
