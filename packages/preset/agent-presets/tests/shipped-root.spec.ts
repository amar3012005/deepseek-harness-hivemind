...
describe('the shipped preset root', () => {
  ...
  it('keeps HIVE chat additive to native prompt guidance and renderer-safe', async () => {
    ...
    expect(prefix).toContain('call hivemind_connected_task search directly')
    ...
    expect(prefix).toContain('request has no concrete referent')
    expect(prefix).toContain('After the primary read succeeds, answer from its receipt')
    expect(prefix).toContain('Request hivemind_capabilities only when')
    expect(prefix).toContain('profiles only for the HyperAgent directory')
    expect(prefix).toContain('keep it to two short sentences')
    expect(prefix).toContain('make at most one materially refined search in the same workflow')
  })
