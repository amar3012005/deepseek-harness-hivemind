...
describe('HIVE runner ticket', () => {
  ...
  it('verifies the fixed issuer, audience, profile, and bounded lifetime', () => {
    ...
    expect(() => verifyTicket(ticket({ exp: now + 61 }), secret, now)).toThrow(/expired_ticket/u)
    expect(() => verifyTicket(ticket({ exp: now }), secret, now)).toThrow(/expired_ticket/u)
  })

  it('rejects tampering and consumes a jti once', async () => {
    const valid = ticket()
    expect(() => verifyTicket(`${valid.slice(0, -1)}x`, secret, now)).toThrow(/invalid_ticket_signature/u)
    ...
    await expect(consumeTicket(valid, secret, consume, now)).rejects.toThrow(/ticket_already_consumed/u)
  })
})
