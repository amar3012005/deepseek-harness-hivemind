...
模型调用 MCP 工具时，调用会以每次调用超时（默认 60 秒）发往远程服务器，并像其他工具调用一样可以取消。结果按块顺序以普通文本返回；资源链接以文本形式显示名称与 URI。如果服务器报告错误，调用会明确失败——模型不会看到虚假的成功。Streamable HTTP 会话丢失或收到可重试的 502、503、504 响应后，桥接会替换失败的协议世代。仅当 MCP 服务器明确声明 `annotations.readOnlyHint: true` 时，同一调用才会重试一次；可写工具和未声明工具不会自动重放。
...
服务器连接断开时——例如本地服务器进程崩溃——插件会以从 500 ms 起逐次翻倍、上限 30 s 的延迟自动重连，并刷新工具集；重连进度在日志中可见。中断期间最后已知的工具仍会列出，但对它们的调用会失败，以上述单次安全重试为例外。连续失败十次后，该服务器的工具会被移除，重连停止，直到你重载配置或重启 harness；服务器持续连接一段时间后，该计数会重置。设置 `reconnect.enabled: false` 可禁用自动重连——此时工具在断开后仍会列出，但调用失败，直到你重载。编辑配置项会在原地重载服务器连接，未变的名称保持不变。
...
工具调用会发送一次未缓存的 `tools/call` 请求，携带原始 MCP 名称、JSON 参数、中止信号与配置的超时；公开名称绝不会发给服务器，也绝不会被解析还原。可重试的 Streamable HTTP 传输失败会轮换受监督的客户端世代。明确标记为只读的工具会等待新世代并将其网络请求重复一次；其他工具不会重放，但轮换仍会修复后续调用。两次尝试仍属于同一次原生 Harness 工具执行，因此保留现有事件、结果与 UI 呈现。规范成功值是 `{ content: JsonValue[], structuredContent? }`，为程序化调用方与 PTC mode 调用方保留完整的 MCP JSON 块。受支持且已声明的 `outputSchema` 会验证 `structuredContent`；不受支持的 schema 词汇回退为不受约束的 `JsonValue`。MCP 的 `isError` 结果会在任何图片持久化之前抛出，使注册表产生失败的工具结果。图片批次会先整体解码并校验，再保存任一成员；任何拒绝都会把每张图片投影为诊断文本。
...
- **安全重试要求明确的只读注解**——Streamable HTTP 会话丢失和可重试的 502、503、504 响应会轮换客户端世代，但只有服务器声明 `annotations.readOnlyHint: true` 时才会重复失败的调用。未提供注解的服务器会为后续调用恢复连接，但不会自动重放。
