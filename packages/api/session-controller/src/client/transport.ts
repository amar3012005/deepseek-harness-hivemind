...
import { adaptHiveWireEvent } from './hive-wire-compat.ts'

function acceptedWireEvent(value: unknown) {
  const event = typeof document !== 'undefined' && document.documentElement.dataset.dshEmbedded === 'true'
    ? adaptHiveWireEvent(value) : value
  assertSessionWireEvent(event)
  return event
}
...
> {
  ...
  protected override async readPage(
    ...
    return { ...result.value, records: result.value.records.map(record => ({ ...record, event: acceptedWireEvent(record.event) })) }
  }
